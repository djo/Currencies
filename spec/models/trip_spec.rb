...
describe Trip do
  ...
  describe ".dates" do
    it "should return ordered unique completed dates" do
      ...
      Factory :trip, :user => user, :completed_at => Date.today
      Factory :trip, :user => user, :completed_at => Date.today
      Factory :trip, :user => user, :completed_at => Date.yesterday
      Factory :trip, :user => user, :completed_at => (Date.today + 10)

      Trip.dates(user).should == [Date.yesterday, Date.today, (Date.today + 10)]
    end
  end
end
